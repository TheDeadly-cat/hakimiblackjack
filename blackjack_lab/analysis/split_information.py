...
from .split_contracts import SplitAnalysisInput, SplitHand, VALUES, supported_split_rules
...
def build_split_input(session_id, current, seat, selected_id, seq, prefix):
    ...
    if not supported_split_rules(rules):
        raise InputUnavailable("SPLIT_RULE_UNSUPPORTED", "两手分析需要明确的顺序、无再分、无DAS、分A一张研究模板", UNSUPPORTED)
    ...
    if any(h.hidden_cards or not h.cards or h.doubled or h.surrendered or h.bet_units != 1 for h in hands):
    ...
    for h in hands:
        ...
        items.append(SplitHand(h.hand_id, h.parent_id, tuple(c.rank for c in h.cards),
            ...
            closed, len(h.cards) == 1 or h.awaiting_hit))
    ...
    if len(hands) == 1:
        ...
        legal = ("complete",) if hand is None else ("deal",) if hand.forced_draw else ("stand", "hit")
