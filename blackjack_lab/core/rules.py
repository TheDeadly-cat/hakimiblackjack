...
CAPABILITY_MATRIX = {
    ...
    "下一轮开局天然BJ概率/开局优势": (EXPERIMENTAL, "独立入口仅精确穷举剩余≤16张、未分牌、无保险；6/7/8副整靴开局仍拒绝；禁止把当前手牌EV改称为开局优势"),
    "整靴发牌前窗口扫描": (EXPERIMENTAL, "剩余>16记未支持；≤16精确或超时；每轮重洗为负对照；可对照固定基本策略与组成策略且不共享实现路径；合成录牌误差有误报/漏报/延迟错过统计；可按独立牌靴汇总覆盖率/零窗口率/收益分布；独立真实录像仍缺；零窗口是合法结果；不构成可靠优势声明"),
    "前三/六轮消耗对照": (EXPERIMENTAL, "6/7/8副、1–7座位按声明策略耗牌；样本单位是牌靴；其他座位不是独立样本、也不是多玩家EV；剩余>16发牌前仍拒绝；固定移除65/130张不是三/六轮"),
    "未知组成区间研究": (EXPERIMENTAL, "须给出候选剩余组成；公布可用EV最小/最大；禁止把平均牌靴写成精确发牌前EV"),
    ...
    "回溯插入漏牌的原子修复": (EXPERIMENTAL, "同一牌靴内、后续不含撤销/纠错/新靴时，预演后缀后一次原子追加UNDO+插入+重放；确认时间不回溯。历史前缀看不到后来插入的牌，禁止用修复后的当前结果宣称当时已抓住窗口"),
