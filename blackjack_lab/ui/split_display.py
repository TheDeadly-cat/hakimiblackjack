...
def format_split_result(result, historical=False):
    ...
    for action,item in result['actions'].items():
        if item['status'] != AVAILABLE:
            ...
            continue
        lines.append(f"{SPLIT_ACTION_ZH[action]} · 合计 EV {item['ev']:+.6f}")
        lines.append(f"  另追加 {item['additional_investment']} · 此动作总投入 {item['total_investment']}")
        if 'hand_evs' in item:
            lines.append('  各手边际 EV：' + ' / '.join(f"{v:+.6f}" for v in item['hand_evs']))
        lines.append('  合计净收益分布：' + '  '.join(f"{float(v):+g}:{p:.2%}" for v,p in item['net_distribution'].items()))
    highest = result.get('highest_ev_action')
    if highest in ('deal','complete'):
        lines.append('当前为确定流程，数值包含其后所有适用的补/停决策。')
    elif highest:
        lines.append('在声明模型与当时信息下合计 EV 最高：'+SPLIT_ACTION_ZH[highest])
    ...
    lines.extend(['', '两手共享剩余牌与同一庄家；无独立卷积。',
