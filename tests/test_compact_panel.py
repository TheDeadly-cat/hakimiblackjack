...
class TestCompactPanel(unittest.TestCase):
    ...
    def test_settlement_recovery_undo_and_new_round_clear_stale_target(self):
        ...
        app.act_undo()
        self.assertIn('庄家 17 点，已自动停牌', app.var_entry_prompt.get())
        self.assertNotIn('下一张给', app.var_entry_prompt.get())
        self.assertIn('当前发牌给：庄家', app.compact_panel.heading.get())
        self.assertTrue(all(button.instate(['disabled'])
                            for button in app.compact_panel.card_buttons))
        reopened_ledger = app.ctrl.ledger.to_list()
        app._key_rank('9')
        self.assertEqual(app.ctrl.ledger.to_list(), reopened_ledger)
        app.act_end_round()
        app.act_new_round()
