...
**简便暗牌录入**：录完初始明牌自动登记一张点数未知的庄家底牌；轮到庄家直接输入底牌点数，随后继续输入补牌，正常流程隐藏“暗牌 .”及揭示模式切换。A/十点明牌仍需按实际检查结果单独确认非BJ；未知、漏录或位置不明回人工核对。Backspace可撤回底牌揭示或玩家最后一张牌。最初约定及历史验收见[简便暗牌说明](docs/UI-simple-hole-entry-20260922.md)，默认启用行为以本段为准。
...
3. 跟随固定队列录入所有玩家第一张、庄家明牌、所有玩家第二张，最后按 `.` 确认庄家暗牌。庄家明牌为 A 或十点时，必须有实际的非 BJ 检查信息，再点“庄家检查底牌：确认非 BJ”。未知结果不要自行勾选。
