...
**简便暗牌录入**：在“录牌设置”勾选“下轮简便暗牌”，确认已核对固定美式初始发牌流程后，下轮录完初始明牌会自动登记未知底牌。轮到庄家直接输入底牌点数，随后继续输入补牌；正常流程隐藏“暗牌 .”及揭示模式切换。A/十点明牌仍需按实际检查结果单独确认非BJ；未知、漏录或位置不明回人工核对。详见[简便暗牌的约定、撤销与验收](docs/UI-simple-hole-entry-20260922.md)。
...
牌面键默认常驻，可收起以便键盘录入；点“录牌设置”展开低频选项，点“展开详情”查看同一计算结果的完整报告；规则、历史、实验与导入导出由“研究工作台”进入。首次使用先进入工作台设置牌靴和座位。三个层级共用现有本地计算与账本。实现及验收范围见[简洁面板说明](docs/UI-compact-decision-20260921.md)。
...
- 同点值分牌请显式选择“同点值”或“同点值DAS”研究模板。旧same_rank模板保持原配对限制；其他真实桌规仍需单独确认。底层默认接口保留`SINGLE_PLAYER_ONLY`门禁；界面显式请求带`per-seat-others-no-draw-v1`身份的条件比较，不冒充完整多人策略EV。
