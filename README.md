...
产品显示版本 `0.2.0b1`。规则模板、求解引擎各有身份：无 DAS 为 `v0.2b1-finite-two-hand-1`；原顺序 DAS 为 `v0.2b2-finite-two-hand-das-2`；常用设置“两手先补齐”的 DAS 为 `finite-two-initial-das-1`。保留 Python、Tkinter 和 SQLite。多人采用逐玩家条件比较：假设其他玩家之后不再补牌，并非整桌后续策略模拟。没有再分、识别、捕获或真实平台接入。按已声明模板计算，不会把旧快照改写为新规则或新引擎。数学与工作流验证、目标机性能、CI 和交付证据分别报告，旧性能矩阵不能替代新顺序的验收。原 DAS 的历史验收见 [V0.2b2 验收与交付](docs/V0.2b2-验收与交付.md)。
...
**分牌先各补一张、21点自动跳过**：新牌盒的常用设置先给两手各发第二张，再依次处理第一手、第二手。分牌EV按这一可见信息顺序计算，两手共享剩余牌盒及庄家。原手或分手达到21点均自动转下一手／下一位；分牌仍会先补齐另一手的初始牌。Backspace可撤回并恢复发牌目标。旧牌盒保留原分牌顺序；要启用新顺序，请用常用设置新建牌靴。见[分牌顺序与21点说明](docs/Both-initial-and-21-20260923.md)。
...
5. 分牌前显示全部合法动作；新常用设置分牌后先给第一手、第二手各补一张，再从第一手开始行动。旧顺序模板仍先完成第一手，再给第二手第二张。21 或爆牌后自动切到下一手。非 A 分手两张可加倍，只补一张后该手闭合，再继续另一手。面板区分选中手和当前行动手；DAS 分开写当前已投入、立刻追加、此后可能再追加，不把分牌后总投入写成固定 2。实际顺序须符合牌盒锁定规则。
