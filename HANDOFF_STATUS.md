...
工程证据：原分支完整532测试，1失败、2跳过；失败是旧模板归一化不一致将合成A排为Q，已修复。修复树首次完整611测试通过、2跳过；随后原帧真值完整性再增2回归，最后聚焦dataset17+frame13共30通过。可选依赖与无依赖环境分别验证；最终数字及精确提交CI另附本机delivery-status.json，不把前次结果换签到后续提交。临时SQLite验证连续40帧双8只扣2、显式新轮同位置再扣2。v3实际WGC只捕获自建Tk窗口，34帧到达/9帧分类/18角标候选/0接受/不入账，窗口关闭；这是接入证据，不是视频准确率。
