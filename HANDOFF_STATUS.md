...
后续反馈已修复“进行中不能换靴”：顶部常驻新建牌靴，确认后原子保存旧轮未结算结束、旧靴关闭及新靴创建；保留旧记录，Backspace组合撤回并恢复旧位置。见[随时换靴](docs/New-shoe-anytime-20260923.md)。继续更新同一草稿 PR #23，不合并 main。
...
最新：顶部增加下一轮参与人数的增减按钮，自动计算默认开启，多人表逐位显示条件动作概率/EV。用户明确选择“其他玩家之后不再补牌、每录一张自动更新”口径；输入与历史保存独立范围身份，真实参与玩家和已知牌全部保留。见[本轮说明](docs/UI-auto-seats-20260921.md)。

练习后反馈修复：顶部实时显示庄家已录牌、点数和当前发牌对象；结算/未结算结束/牌靴结束后从账本状态清除旧录牌目标提示，支持撤销与恢复。新建默认8副牌；Tab/Enter及反向键按参与顺序循环，Ctrl+1～7/Ctrl+0直达玩家/庄家。详情见[练习反馈验收](docs/UI-practice-followup-20260921.md)。仍更新PR #15供审查，不合并main。

独立分支`codex/ui-compact-decision-20260921`，基线PR #14的1cc8c65。默认小面板展示当前行动手、EV排序的两项选择、净盈利概率和有效状态；详情和工作台按需进入，共用一个AnalysisPanel／计算任务。记录、取消、超时、纠错和历史失效机制保留。完整需求对照、实测范围与复现见[UI验收说明](docs/UI-compact-decision-20260921.md)。数学、规则、精度、Tk框架和存储未改，main不合并。

---

# 历史接手：K-F5 手动流程收口（2026-09-21）

补充：用户改由Computer Use接手，已在5004d79真实窗口完成两轮、暂停/撤销、分牌/DAS、庄家切换及34条事件重启恢复。发现并修复撤销续牌后“刚刚记入”残留提示，48项集中回归通过。真实操作与修复后的工具限制见[Computer Use核对](docs/KF5-computeruse-20260921.md)，不能记作真人体验。

基线79f4793，独立工作目录`pr14-kf5-finish`。补齐最后玩家收牌结束后切庄家，以及最近保存状态的座位归属；真实窗口发现提示裁切，已将完整进度提示移至牌桌上方。数值与账本规则不变。详细证据范围、命令及回滚方式见[本轮验收说明](docs/KF5-flow-closeout-20260921.md)。

当前目标为同一PR #14的增量更新，main继续不合并。自动连续演练111张新可见牌与三份备份恢复通过；真人记录使用单独临时库，反馈与完成范围按实际回执报告。下文为此前交接，不能用旧CI替代当前HEAD。

---

# 当前接手状态：PR14 K-F1～K-F4（2026-09-21）

用户在完成本地修复与验收后，追加授权“更新项目仓库”：将K-F1～K-F4提交并同步到现有PR #14的`codex/k1-k2-fixed-initial-deal-r4`分支，继续不合并main。独立修复目录为`pr14-kf-reliability`，本地分支`codex/pr14-kf-reliability-20260920`，修复基线为`458bab837737ad724a878d667b8659cefecb8746`。此前审查包保留原字节，仍表示提交前的本地验收状态；当前提交与远程CI以PR #14为准。

已实现：提交后计划协调与前缀核对、输入暂停、控件前键盘绑定、按具体手牌导航及恢复、同点值真实求解与历史复算验收。SQLite schema 2与数值引擎不变。旧计划恢复需人工核对，不猜测归属。详情及真实验证范围见[PR14 K-F修复与验收](docs/PR14-KF修复与验收-20260921.md)。

下一项是A1多人桌上的本人目标分析，先定义信息集、后序玩家行为与验证契约；现有SINGLE_PLAYER_ONLY继续保留。下文均为历史交接，不作为本轮新任务或授权。
